...
BOOL CHttpSocket::Socket()
{
	...
	{
		//MessageBox(NULL,L"socket()函数执行失败!",L"错误",MB_OK);
		return FALSE;
	}
...
BOOL CHttpSocket::Connect(char *szHostName,int nPort)
{
	...
	if(m_phostent==NULL)
	{
		//MessageBox(NULL,L"gethostbyname()函数执行失败!",L"错误",MB_OK);
		return FALSE;
	}
	...
	*/
	...
	if(connect(m_s,(struct sockaddr*)&destaddr,sizeof(destaddr))!=0)
	{
		//CloseSocket();
		//m_s=NULL;
		//MessageBox(NULL,L"connect()函数执行失败!",L"错误",MB_OK);
		return FALSE;
	}
...
BOOL CHttpSocket::SendRequest(const char *pRequestHeader, long Length)
{
	...
	{
		//MessageBox(NULL,L"send()函数执行失败!",L"错误",MB_OK);
		return FALSE;
	}
	int nLength;
...
//设置接收或者发送的最长时间
BOOL CHttpSocket::SetTimeout(long nTime, int nType)
{
	if(nType == 0)
	{
		nType = SO_RCVTIMEO;
...
int CHttpSocket::GetField(const char *szSession, char *szValue, int nMaxLength)
{
	...
	int nPos = -1;
	nPos = strRespons.Find(A2T(szSession),0);
	if(nPos != -1)
	{
